 ...
 *  usage:
 *   $options = array(
 *			'token'=>'tokenaccesskey', //填写应用接口Token
...
class Corp
{
    ...
    const EVENT_MENU_PIC_SYS 		= 'pic_sysphoto';       //菜单 - 系统拍照发图
    ...
    const MATERIAL_ADD_MPNEWS   = '/material/add_mpnews?';
    const MATERIAL_UPLOAD_MPNEWS = '/material/update_mpnews?';
    const MATERIAL_ADD_MATERIAL = '/material/add_material?';
    const MATERIAL_DELETE       = '/material/del?';
    ...
    private $suiteAccessToken;  // 应用套件令牌
    private $authCorpAccessToken; // 授权企业token
    ...
    public function __construct($options)
    {
        ...
        $this->suiteId = isset($options['suiteId'])?$options['suiteId']:'';
        $this->authCorpId = isset($options['authCorpId'])?$options['authCorpId']:'';
        $this->permanentCode = isset($options['permanentCode'])?$options['permanentCode']:'';
        $this->suiteSecret = isset($options['suiteSecret'])?$options['suiteSecret']:'';
     ...
     * @return bool
     */
    public function createWxMenu($menu,$agentId){
        if ($agentId=='') {
        ...
        $result = $curl->post(self::API_URL_PREFIX.self::MENU_CREATE_URL.'access_token='.$this->access_token.'&agentid='.$agentId,json_encode($menu,JSON_UNESCAPED_UNICODE));
    ...
    public function valid($return=false)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            $this->appid = $array['ToUserName'];

            $this->log('postStrXML:');
            $this->log($postStr);
            $this->log('postStrArray:');
            $this->log($array);
        ...
        /* if (isset($array['ToUserName']) && $array['ToUserName'] != $this->appid) {
             $appId = $this->suiteId;
         } else {
             $appId = $this->appid;
         }*/
    ...
    {
        if (isset($this->_receive['SuiteId'])) {
            return $this->_receive['SuiteId'];
        } else {
            return false;
        }
    }
     ...
     * @return bool
     */
    public function getSuiteInfoType()
    {
    ...
    {
        if (isset($this->_receive['SuiteTicket'])) {
            return $this->_receive['SuiteTicket'];
        } else {
            return false;
        }
    }
    ...
    {
        if (isset($this->_receive['AuthCorpId'])) {
            return $this->_receive['AuthCorpId'];
        } else {
            return false;
        }
    }
    ...
    {
        if (isset($this->_receive['AuthCode'])) {
            return $this->_receive['AuthCode'];
        } else {
            return false;
        }
    }
    ...
    public function getSuiteAccessToken(){
        $authName = 'qywechat_suite_access_token'.$this->suiteId;
        if ($rs = $this->getCache($authName))  {
            $this->suiteAccessToken = $rs;
            return $rs;
        }
        ...
        $param = array(
            ...
            'suite_ticket' => S('suite_ticket')
        );

        $result = $this->http_post('https://qyapi.weixin.qq.com/cgi-bin/service/get_suite_token',json_encode($param));
        ...
        if ($result) {
            ...
            $this->suiteAccessToken = $json['suite_access_token'];
            $expire             = $json['expires_in'] ? intval($json['expires_in']) - 100 : 3600;
            $this->setCache($authName, $this->suiteAccessToken, $expire);
     ...
     * @param $permanentCode
     *
     * @author liangh.zhu <liangh.zhu@gmail.com>
     *
     * @return bool|mixed
     */
    public function getAuthCorpAccessToken($authCorpId,$permanentCode)
    {
        if (!$this->suiteAccessToken && !$this->getSuiteAccessToken()) {
            return false;
        }

        $authName = 'qywechat_auth_corp_access_token'.$this->authCorpId;
        if ($rs = $this->getCache($authName))  {
            $this->authCorpAccessToken = $rs;
            return $rs;
        }
        ...
        $result = $this->http_post('https://qyapi.weixin.qq.com/cgi-bin/service/get_corp_token?suite_access_token='.$this->suiteAccessToken,json_encode($param));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
            ...
            $this->authCorpAccessToken = $json['access_token'];
            $expire                    = $json['expires_in'] ? intval($json['expires_in']) - 100 : 3600;
            $this->setCache($authName, $this->access_token, $expire);

            return $this->authCorpAccessToken;
        }
    ...
    public function getPreAuthCode()
    {
        ...
        $result = $this->http_post('https://qyapi.weixin.qq.com/cgi-bin/service/get_pre_auth_code?suite_access_token='.$this->suiteAccessToken,json_encode($param));
     ...
     * @return bool|mixed
     */
    public function getPermanentAuthCode($authCode = '')
    {
        if (!$this->suiteAccessToken && !$this->getSuiteAccessToken()) {
            return false;
        }
        ...
        $param = array(
            'suite_id' => $this->suiteId,
            'auth_code' => empty($authCode) ? $this->getAuthCode() : $authCode
        );
        $result = $this->http_post('https://qyapi.weixin.qq.com/cgi-bin/service/get_permanent_code?suite_access_token='.$this->suiteAccessToken,json_encode($param));
     ...
     * @param string $permanentAuthCode
     *
     * @author liangh.zhu <liangh.zhu@gmail.com>
     *
     * @return bool|mixed
     */
    public function getAuthInfo($authCorpId = '', $permanentAuthCode = '')
    {
        if (!$this->suiteAccessToken && !$this->getSuiteAccessToken()) {
            return false;
        }
        ...
        $param = array(
            'suite_id'       => $this->suiteId,
            'auth_corpid'    => $authCorpId,
            'permanent_code' => $permanentAuthCode
        );
        $result = $this->http_post('https://qyapi.weixin.qq.com/cgi-bin/service/get_auth_info?suite_access_token='.$this->suiteAccessToken,json_encode($param));
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function addMaterial($data, $type){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::MATERIAL_ADD_MATERIAL.'access_token='.$this->access_token.'&type='.$type,$data,true);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return string
     */
    public function addNewsMedia(array $data)
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $tmp = array(
            'mpnews' => array('articles' => $data)
        );
        $result = $this->http_post(self::API_URL_PREFIX.self::MATERIAL_ADD_MPNEWS.'access_token='.$this->access_token,self::json_encode($tmp));
     ...
     * @return bool|mixed
     */
    public function editNewsMedia($media_id, $data)
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $tmp = array(
            'media_id' => $media_id,
        ...
        $result = $this->http_post(self::API_URL_PREFIX.self::MATERIAL_UPLOAD_MPNEWS.'access_token='.$this->access_token,self::json_encode($tmp));
    ...
    public function deleteMedia($media_id)
    {
        ...
        $result = $this->http_get(self::API_URL_PREFIX.self::MATERIAL_DELETE.'access_token='.$this->access_token.'&media_id='.$media_id);
    ...
    public function createDepartment($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::DEPARTMENT_CREATE_URL.'access_token='.$this->access_token,self::json_encode($data,JSON_UNESCAPED_UNICODE));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function updateDepartment($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::DEPARTMENT_UPDATE_URL.'access_token='.$this->access_token,self::json_encode($data,JSON_UNESCAPED_UNICODE));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function moveDepartment($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::DEPARTMENT_MOVE_URL.'access_token='.$this->access_token,self::json_encode($data,JSON_UNESCAPED_UNICODE));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function createUser($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::USER_CREATE_URL.'access_token='.$this->access_token,self::json_encode($data,JSON_UNESCAPED_UNICODE));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function updateUser($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::USER_UPDATE_URL.'access_token='.$this->access_token,self::json_encode($data,JSON_UNESCAPED_UNICODE));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function deleteUsers($userids){
        ...
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::USER_BATCHDELETE_URL.'access_token='.$this->access_token,self::json_encode($data,JSON_UNESCAPED_UNICODE));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function getSuiteUserId($code){

        $result = $this->http_get(self::API_URL_PREFIX.self::USER_GETINFO_URL.'access_token='.$this->authCorpAccessToken.'&code='.$code);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function getSuiteUserInfo($userId)
    {
        $result = $this->http_get(self::API_URL_PREFIX.self::USER_GET_URL.'access_token='.$this->authCorpAccessToken.'&userid='.$userId);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function sendInvite($userid,$invite_tips=''){
        ...
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::USER_INVITE_URL.'access_token='.$this->access_token,self::json_encode($data,JSON_UNESCAPED_UNICODE));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function createTag($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::TAG_CREATE_URL.'access_token='.$this->access_token,self::json_encode($data,JSON_UNESCAPED_UNICODE));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function updateTag($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::TAG_UPDATE_URL.'access_token='.$this->access_token,self::json_encode($data,JSON_UNESCAPED_UNICODE));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function addTagUser($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::TAG_ADDUSER_URL.'access_token='.$this->access_token,self::json_encode($data,JSON_UNESCAPED_UNICODE));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function delTagUser($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::TAG_DELUSER_URL.'access_token='.$this->access_token,self::json_encode($data,JSON_UNESCAPED_UNICODE));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function sendMessage($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::MASS_SEND_URL.'access_token='.$this->access_token,self::json_encode($data,JSON_UNESCAPED_UNICODE));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
     ...
     * @version 0.0.0.1
     */
    public function setAgent($data){//access_token=ACCESS_TOKEN
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::AGENT_SET_URL.'access_token='.$this->access_token,self::json_encode($data,JSON_UNESCAPED_UNICODE));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
